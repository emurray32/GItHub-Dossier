...
_fpdf_available = _check_fpdf_conflict()

# Only import FPDF if no conflicts detected
from datetime import datetime
import os

# Only define ReportPDF class if FPDF is available
ReportPDF = None
...
def generate_report_pdf(report, output_path):
    ...
    Raises:
        RuntimeError: If PDF packages are conflicting or unavailable
    """
    if not _fpdf_available or ReportPDF is None:
        raise RuntimeError(
            f"PDF generation unavailable: {_fpdf_conflict_error or 'fpdf2 package not installed'}"
        )

...
def get_or_create_inbox():
    ...
    try:
        inboxes = list(client.inboxes.list())
        if inboxes:
            _cached_inbox = inboxes[0]
            print(f"[AGENTMAIL] Using existing inbox: {_cached_inbox.email}")
            return _cached_inbox
        
        _cached_inbox = client.inboxes.create()
        print(f"[AGENTMAIL] Created new inbox: {_cached_inbox.email}")
        return _cached_inbox
